...
data "aws_ami" "debian" {
  ...
  filter { name = "name" values = ["debian-13-amd64-*"] }
}

resource "aws_security_group" "comportamiento_sg" {
  name        = "comportamiento-sg"
  description = "SSH + 8000"
  vpc_id      = data.aws_vpc.default.id

  ingress { description="SSH" from_port=22 to_port=22 protocol="tcp"
    cidr_blocks=[var.open_to_world ? "0.0.0.0/0" : var.my_ip_cidr] }
  ingress { description="comportamiento" from_port=8000 to_port=8000 protocol="tcp"
    cidr_blocks=[var.open_to_world ? "0.0.0.0/0" : var.my_ip_cidr] }
  egress  { description="all egress" from_port=0 to_port=0 protocol="-1"
...
resource "aws_instance" "comportamiento" {
  ...
  user_data = <<-EOF
    ...
    USERNAME="debian"; id admin && USERNAME="admin" || true
  ...
  tags = { Name = "aldana-comportamiento" }
}

# Optional: Elastic IP (descomenta si quieres IP fija)
